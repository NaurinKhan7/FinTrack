...
export default function BudgetScreen() {
  ...
  // Update the fetchData function in budget.tsx
const fetchData = useCallback(async () => {
  try {
    ...
    // First get budget status which includes transactions
    const budgetStatusResponse = await fetch(`${API_URL}/transactions/budget-status`, { 
      headers 
    });
    
    if (!budgetStatusResponse.ok) throw new Error('Failed to fetch budget status');
    const budgetStatus = await budgetStatusResponse.json();

    // Then get the budgets themselves
    ...
    const budgetsWithStatus = budgetsData.map((budget: Budget) => {
      const status = budgetStatus.categorySpending[budget.category] || {
    ...
    setTotalIncome(budgetStatus.totalIncome || 0);
    setTotalExpenses(budgetStatus.totalSpent || 0);
  } catch (err: any) {
